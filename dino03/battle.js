...
export class BattleSystem {
    constructor(ui, cameraControl) { this.ui = ui; this.cameraControl = cameraControl; }
    executeAttack(attacker, defender, units, camera, onComplete) {
        attacker.lookAtNode(defender.x, defender.z); defender.lookAtNode(attacker.x, attacker.z);
        if(attacker.texture) attacker.setAction('ATTACK'); if(defender.texture) defender.setAction('HURT');
        const damage = Math.max(1, (attacker.str - defender.def) + (attacker.h - defender.h) * 2);
        const origPos = attacker.sprite.position.clone();
        const dx = (defender.sprite.position.x - attacker.sprite.position.x) * 0.6, dz = (defender.sprite.position.z - attacker.sprite.position.z) * 0.6;
        const tl = gsap.timeline({ onComplete: () => {
            defender.hp -= damage; this.ui.showFloatingText(defender, damage, 'damage', camera);
            if(attacker.texture) attacker.setIdle(); if(defender.texture && defender.hp > 0) defender.setIdle();
            if(defender.hp <= 0) {
                if(defender.texture) defender.setAction('DOWN');
                if(attacker.isPlayer) { attacker.exp += 100; attacker.levelUp(); setTimeout(() => { this.ui.showFloatingText(attacker, "LEVEL UP!", 'levelup', camera); this.ui.showStatus(attacker); }, 400); }
    ...
    executeMovement(unit, path, onComplete, followCamera = false) {
        const tl = gsap.timeline({ onComplete: () => { const last = path[path.length - 1]; unit.x = last.x; unit.z = last.z; unit.h = last.h; onComplete(); }});
        path.forEach(step => {
            const tX = (step.x * 60) - (25 * 60) / 2, tZ = (step.z * 60) - (25 * 60) / 2, tY = step.h * 30;
    ...
    decideEnemyAI(units, player) {
        ...
        return enemies.filter(e => {
            ...
            return (e.id === 'ブラキオサウルス' && !anyComp_alive);
        });
    }
}
