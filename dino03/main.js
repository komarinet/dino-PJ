...
function init(sheetImg, braTex, rexTex, compTex) {
    ...
    window.units = StageData.units.map(u => {
        ...
        unit.h = mapData[unit.z][unit.x].h; scene.add(unit.sprite); updateUnitPos(unit);
        if(unit.isPlayer) window.player = unit; return unit;
    });
    setupEventListeners();
    animate();
    playStageTitle(() => {
        cameraCtrl.playOpening(window.units.find(u => u.id === 'ブラキオサウルス'), window.player, () => {
...
function startEvent(talkData, callback) {
    uiCtrl.hideAll(); gameStore.setState({ gameState: 'TALKING', talkIndex: 0 });
    cameraCtrl.setZoom(2.5); document.getElementById('event-ui').style.display = 'flex';
    window.onGlobalTap = () => {
        const idx = getStore().talkIndex + 1;
        if(idx < talkData.length) { gameStore.setState({ talkIndex: idx }); uiCtrl.renderTalkLine(talkData[idx], window.units, window.player); }
        else { document.getElementById('event-ui').style.display = 'none'; cameraCtrl.setZoom(1.5); gameStore.setState({ gameState: 'IDLE' }); callback(); }
    };
...
function startPlayerTurn() { gameStore.setState({ gameState: 'IDLE' }); uiCtrl.hideAll(); clearHighlights(); uiCtrl.setMsg("あなたの番です！", "#00ff00"); }
...
function answerConfirm(isYes) {
    ...
    if(store.confirmMode === 'MOVE') battleSys.executeMovement(window.player, store.pendingData, () => { window.player.hasMoved = true; startPlayerTurn(); });
...
function checkVictory() {
    ...
    startEvent(StageData.postBattleTalk, () => {
        ...
        battleSys.executeMovement(window.player, exitPath, () => { gsap.to(window.player.sprite.scale, {x:0, y:0, duration:0.5}); document.getElementById('episode-clear-overlay').style.opacity = 1; }, true);
    });
}

function updateUnitPos(u) { const offX = (25 * 60) / 2, offZ = (25 * 60) / 2; u.sprite.position.set((u.x * 60) - offX, u.h * 30, (u.z * 60) - offZ); }
...
function playStageTitle(cb) { const o = document.getElementById('stage-overlay'); document.getElementById('chapter-num').innerText = StageData.info.chapter; document.getElementById('stage-title').innerHTML = StageData.info.name; gsap.to(o, { opacity: 1, duration: 1.5, onComplete: () => gsap.to(o, { opacity: 0, delay: 2.0, onComplete: cb }) }); }
