...
window.tilesMeshMap = {}; window.interactableTiles = []; window.units = [];
...
function init(sheetImg, braTex, rexTex, compTex) {
    ...
    playStageTitle(() => {
        cameraCtrl.playOpening(window.units.find(u => u.id === 'ブラキオサウルス'), window.player, () => {
            startEvent(window.StageData.preBattleTalk, playBattleStart);
        });
    });
}
...
function onPointerClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        if(store.gameState === 'IDLE') {
            ...
            if(route) {
                clearHighlights();
                gameStore.getState().setGameState('CONFIRMING');
                gameStore.getState().setConfirmData('MOVE', route.path, `${route.x},${route.z}`);
...
function startPlayerTurn() {
    gameStore.getState().setGameState('IDLE');
...
function execCommand(cmd) {
    if(cmd === 'move') {
        gameStore.getState().setGameState('SELECTING_MOVE');
        const tiles = window.getWalkableNodes(window.player);
        gameStore.getState().setWalkableTiles(tiles);
        tiles.forEach(node => { 
        ...
        gameStore.getState().setGameState('SELECTING_ATTACK_TARGET');
        ...
        gameStore.getState().setConfirmData('WAIT', null);
...
function selectTarget(t) {
    gameStore.getState().setConfirmData('ATTACK', t);
...
function answerConfirm(isYes) {
    ...
    gameStore.getState().setGameState('ANIMATING');
...
function endPlayerTurn() {
    ...
    gameStore.getState().setGameState('ENEMY_TURN');
...
async function processEnemyAI() {
    ...
    for(const enemy of enemies) {
        const routes = window.getWalkableNodes(enemy);
...
function checkVictory() {
    gameStore.getState().setGameState('FINISHED');
    startEvent(window.StageData.postBattleTalk, () => {
...
function updateUnitPos(u) {
    const offX = (window.MAP_W * window.TILE_SIZE) / 2, offZ = (window.MAP_D * window.TILE_SIZE) / 2;
    u.sprite.position.set((u.x * window.TILE_SIZE) - offX, u.h * window.H_STEP, (u.z * window.TILE_SIZE) - offZ);
}
