...
export class CameraControl {
    constructor(camera, controls) { this.camera = camera; this.controls = controls; }
    centerOn(pos, dur = 0.8) { gsap.to(this.controls.target, { x: pos.x, z: pos.z, duration: dur, ease: "power1.inOut" }); }
    setZoom(z, dur = 1.2) { gsap.to(this.camera, { zoom: z, duration: dur, onUpdate: () => this.camera.updateProjectionMatrix() }); }
    rotate(deg) {
        const r = this.controls.getDistance(), polar = this.controls.getPolarAngle(), startAz = this.controls.getAzimuthalAngle();
    ...
    pan(dx, dy) {
        const panScale = 150, ang = this.controls.getAzimuthalAngle();
        const moveX = (dx * Math.cos(ang) + dy * Math.sin(ang)) * panScale, moveZ = (-dx * Math.sin(ang) + dy * Math.cos(ang)) * panScale;
    ...
    playOpening(boss, player, callback) {
        gsap.to(this.controls.target, { x: boss.sprite.position.x, z: boss.sprite.position.z, duration: 3.0, ease: "power2.inOut", onComplete: () => {
            setTimeout(() => { gsap.to(this.controls.target, { x: player.sprite.position.x, z: player.sprite.position.z, duration: 2.0, ease: "power2.inOut", onComplete: callback }); }, 1000);
        }});
    }
}
